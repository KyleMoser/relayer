...
import (
	...
	"github.com/strangelove-ventures/lens/client/tx"
	lensCmd "github.com/strangelove-ventures/lens/cmd"
	"go.uber.org/zap"
)
...
func feegrantConfigureBasicCmd(a *appState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ctx := cmd.Context()
			_, err = tx.EnsureBasicGrants(ctx, memo, &prov.ChainClient)
			if err != nil {
...
func feegrantBasicGrantsCmd(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			pageReq, err := lensCmd.ReadPageRequest(cmd.Flags())
			if err != nil {
				return err
			}
			...
			q := &query.Query{Client: &prov.ChainClient, Options: &options}
			...
			for _, grant := range res {
				allowance, e := prov.MarshalProto(grant.Allowance)
				cobra.CheckErr(e)
...
func TestFeegrant(log *zap.Logger, home string, chain string, granterKeyOrAddr string, update bool, updateGrantees bool, numGrantees int, grantees []string) error {
   ...
   NOTE: Most of the commands have aliases that make typing them much quicker 
	...
	_, err = tx.EnsureBasicGrants(ctx, "", &prov.ChainClient)
	if err != nil {
		return fmt.Errorf("error writing grants on chain: '%s'", err.Error())
	}
